...
############################################################
#
# Copyright 2017, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026
# Mohammed El-Afifi
...
class TestFlow:
    ...
    def test_earlier_instructions_are_propagated_first(self):
        ...
        """
        in_units = [
            UnitModel(name, 1, [categ], LockInfo(True, False), [])
            for name, categ in [("ALU input", "ALU"), ("MEM input", "MEM")]
        ]
        out_unit = FuncUnit(
            UnitModel("output", 1, ["ALU", "MEM"], LockInfo(False, True), []),
            in_units,
        )
        assert simulate(
...
class TestInSort:
    ...
    def test_inputs_are_lexicographically_sorted(self):
        ...
        """
        in_unit, out_unit = (
            UnitModel(name, 1, ["ALU"], LockInfo(rd_lock, wr_lock), mem_acl)
        ...
        proc_desc = ProcessorDesc(
            ...
            [UnitModel("input 2", 1, ["ALU"], LockInfo(True, False), [])],
            [],
        )
        assert simulate(
...
class TestStall:
    ...
    def test_internal_stall_is_detected(self):
        ...
        in_unit, mid, out_unit = (
            UnitModel(name, width, ["ALU"], LockInfo(rd_lock, wr_lock), [])
